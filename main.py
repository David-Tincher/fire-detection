...
if os.path.isdir('train/fire') is False:
    ...
    # There are 360 images. fire(180) & non_fire(180)
    # Train : Valid : Test = 8 : 2 : 2
    # Train: 240 (120, 120)
    # Valid: 60 (30, 30)
    # Test: 60 (30, 30)
    for c in random.sample(glob.glob('fire*'), 120):
        shutil.move(c, 'train/fire')
    for c in random.sample(glob.glob('non_fire*'), 120):
        shutil.move(c, 'train/non_fire')
    for c in random.sample(glob.glob('fire*'), 30):
        shutil.move(c, 'valid/fire')
    for c in random.sample(glob.glob('non_fire*'), 30):
